...
def read_csv_as_df():
    ...
    df = pd.read_csv(
        config["READ_CSV"]["csv_location"],
        usecols=fields,
        nrows=int(config["READ_CSV"]["nrows"]),
        skiprows=range(int(sys.argv[3]), int(sys.argv[4])),
    )
    df.info()
...
def json_to_jsonl(d_json):
    d_json_convert = json.loads(d_json)
    # convert json dump to jsonl format of Doccano
    df_annotated = pd.DataFrame.from_dict(
        json_normalize(d_json_convert), orient="columns"
    )
    if not os.path.exists("auto_annotated_data"):
        os.mkdir("auto_annotated_data")
    df_annotated.to_json(
        "auto_annotated_data\\" + sys.argv[2], orient="records", lines=True
    )


start_time = time.time()
df_dict = read_csv_as_df()
randomize_list(df_dict)
d_json = auto_annotate_data(df_dict, sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
json_to_jsonl(d_json)

print(
    "Auto annotated training data using {} in {} seconds".format(

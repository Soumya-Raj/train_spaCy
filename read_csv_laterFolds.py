...
def randomize_list(df_list):
    ...
    for i in second_half:
        df_list[i]['Search Query'] = df_list[i]['Search Query'] + " 12 fl.oz"

fields = ['Search Query']
df = pd.read_csv('ORS_serachData\\search.csv', usecols=fields, nrows=100, skiprows=range(1,305))
df.info()
...
df_new.head()
df_new.to_json('auto_annotated_data\\auto_annotated_fold4.jsonl',orient="records", lines=True)
   
print("Annotated second fold of TRAIN_DATA using partially trained model in {}".format(time.time() - start))
